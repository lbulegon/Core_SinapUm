...
class VendedorAgent(AgnosticAgent):
    ...
    def _handle_ask_price(
        ...
        # Buscar preço do produto do contexto (offer_id)
        preco = None
        moeda = "BRL"
        ...
        if context.offer_id:
            try:
                # Buscar dados da oferta no Django Évora
                evora_url = settings.EVORA_API_URL.rstrip('/')
                url = f"{evora_url}/api/oferta-produto/{context.offer_id}/"
                
                headers = {}
                if settings.EVORA_API_KEY:
                    headers['Authorization'] = f"Bearer {settings.EVORA_API_KEY}"
                
                response = requests.get(url, headers=headers, timeout=5)
                
                if response.status_code == 200:
                    data = response.json()
                    preco = data.get('preco_exibido')
                    moeda = data.get('moeda', 'BRL')
                    produto_data = data.get('produto', {})
                    if produto_data:
                        nome_produto = produto_data.get('nome_produto', 'o produto')
                    
                    self.logger.info(f"Preço encontrado para oferta {context.offer_id}: {preco} {moeda}")
                else:
                    self.logger.warning(f"Erro ao buscar oferta {context.offer_id}: {response.status_code}")
            except Exception as e:
                self.logger.error(f"Erro ao buscar preço da oferta: {str(e)}", exc_info=True)
        ...
        if preco:
            try:
                ...
                if moeda == "BRL":
                    ...
                    preco_str = f"{preco_float:,.2f}"
                    preco_formatado = f"R$ {preco_str.replace(',', 'X').replace('.', ',').replace('X', '.')}"
                else:
                    preco_formatado = f"{moeda} {preco_float:,.2f}"
                
                response_msg = f"{nome_produto.capitalize()} está por *{preco_formatado}*.\n\n"
            except (ValueError, TypeError):
                # Se não conseguir converter, usar valor direto
                response_msg = f"{nome_produto.capitalize()} está por *{moeda} {preco}*.\n\n"
        else:
            response_msg = "Vou verificar o preço para você. Um momento...\n\n"
            response_msg += "O produto está disponível. "
        ...
        return AgentResponse(
            message=response_msg,
            action="ask_price",
            data={"preco": str(preco) if preco else None, "moeda": moeda}
        )
